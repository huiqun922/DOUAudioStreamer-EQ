...
@interface PlayerViewController ()<UIPickerViewDelegate,UIPickerViewDataSource> {
@private
  UILabel *_titleLabel;
  ...
  NSInteger selectedEQPresetIndex;

}

@property (strong,nonatomic)UIView *EQContentView;
...
- (void)loadView
{
  ...
  [_buttonStop setFrame:CGRectMake(80.0, CGRectGetMaxY([_buttonNext frame]) + 20.0, 60.0, 20.0)];
  ...
  [view addSubview:_buttonStop];
    ...
    _buttonEQ = [UIButton buttonWithType:UIButtonTypeSystem];
    [_buttonEQ setFrame:CGRectMake(CGRectGetWidth([view bounds]) - 80.0 - 60.0, CGRectGetMaxY([_buttonNext frame]) + 20.0, 160.0, 20.0)];
    [_buttonEQ setTitle:@"EQ" forState:UIControlStateNormal];
    [_buttonEQ addTarget:self action:@selector(_actionEQ:) forControlEvents:UIControlEventTouchDown];
    [view addSubview:_buttonEQ];
...
- (void)_actionEQ:(id)sender
{

    
    DLAlertView *alertView = [[DLAlertView alloc] initWithWithContentView:[self setupEQPickView] clickCallBack:^{
...
-(UIView *)setupEQPickView{
    
    if(self.EQContentView){
        
        return self.EQContentView;
    }
    
    CGFloat scale=kWidth/320.0;
    
    CGFloat circleSize=220.0 * scale;
    if (kHeight==480.0) {
        circleSize=200.0 * scale;
    }
    
    self.EQContentView=[[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, kWidth * 0.76, kWidth * 0.76 /0.74)];
    
    UIPickerView *pickView=[[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 0.0, kWidth * 0.76, kWidth * 0.76 /0.74)];
    pickView.dataSource=self;
    pickView.delegate=self;
    [pickView selectRow:selectedEQPresetIndex inComponent:0 animated:NO];
    [self.EQContentView addSubview:pickView];
...
- (NSInteger)pickerView:(UIPickerView *)thePickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    return (CFArrayGetCount([DOUAudioEventLoop sharedEventLoop].mEQPresetsArray));
}
...
- (NSString *)pickerView:(UIPickerView *)thePickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    currentPreset = (AUPreset *)CFArrayGetValueAtIndex([DOUAudioEventLoop sharedEventLoop].mEQPresetsArray , row);
    
    return (__bridge NSString *)currentPreset->presetName;
}

- (void)pickerView:(UIPickerView *)thePickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    
    selectedEQPresetIndex = row;
    
    [[DOUAudioEventLoop sharedEventLoop] selectEQPreset:selectedEQPresetIndex];
    
    AUPreset *currentPreset;
    
    currentPreset = (AUPreset *)CFArrayGetValueAtIndex([DOUAudioEventLoop sharedEventLoop].mEQPresetsArray , row);
    
    NSString *strName = (__bridge NSString *)currentPreset->presetName;
    
    [_buttonEQ setTitle:strName forState:UIControlStateNormal];
