...
- (void)start
{
  if (_exportSession != nil) {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[self cachedPath]]) {
        ...
        NSDictionary * attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:_cachedPath error:nil];
        
        // file size
        NSNumber *theFileSize;
        if ((theFileSize = [attributes objectForKey:NSFileSize])){
          NSInteger filesize =  [theFileSize integerValue];
            NSLog(@"文件大小 %d",filesize);
        if (filesize>0) {
